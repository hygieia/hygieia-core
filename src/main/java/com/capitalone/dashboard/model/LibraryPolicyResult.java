...
public class LibraryPolicyResult extends BaseModel {
    ...
    public void addThreat(LibraryPolicyType type, LibraryPolicyThreatLevel level, LibraryPolicyThreatDisposition disposition, String dispositionStatus, String component,String age) {
        ...
        if (CollectionUtils.isEmpty(threatSet)) {
            Threat threat = new Threat(level, 1);
            threat.getComponents().add(getComponentPlusDispositionPlusAge(component,dispositionStatus,age));
            ...
            for (Threat t : threatSet) {
                if (Objects.equals(t.getLevel(), level)) {
                    ...
                    if (!t.getComponents().contains(getComponentPlusDispositionPlusAge(component,dispositionStatus,age))) {
                        t.getComponents().add(getComponentPlusDispositionPlusAge(component,dispositionStatus,age));
                    }
                    threatSet.add(t);
                    threats.put(type, threatSet);
                    found = true;
    ...
    private String getComponentPlusDispositionPlusAge (String component, String dispositionStatus,String age) {
        return String.format("%s##%s##%s", component, dispositionStatus, age);
    }
